...
def page_mri_visualizer_body():
    ...
    st.info(
        ...
        'differentiates a healthy brain MRI scan from the one with tumor .')
    version = 'v4'
    if st.checkbox('Average and Variabilitiy images'):
        ...
        st.warning(
          ...
          'general patterns where we could intuitively differentiate one from '
          'another. However, there are cases where that difference is not so '
          'obvious and that is due to differences in the development, size '
          'and age of the brain. These factors are making the prediction '
          'quite challenging.')

        st.image(avg_non_tumor,
                 caption='Healthy brain MRI - Average and Variability')
        st.image(avg_tumor,
                 caption='Brain MRI with tumor - Average and Variability')
        st.write('---')

    if st.checkbox(
          'Differences between average brain MRI scan without and with tumor'):
        diff_between_avgs = plt.imread(f'outputs/{version}/avg_diff.png')

        st.warning(
          '* We notice this study did show some general '
          'patterns where we could intuitively differentiate one from '
          'another. But that is not always the case.')
...
def image_montage(dir_path, label_to_display, nrows, ncols, figsize=(15, 10)):
    '''
    Plots and image montage of the selected label
    '''
